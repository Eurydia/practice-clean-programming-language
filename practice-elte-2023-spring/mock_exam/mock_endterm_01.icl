...
getAvg :: IntList -> Real
getAvg    ls      =  avg (map toReal ls) 

getBestBScStudent :: [Student] -> String
getBestBScStudent    []        = "ERROR"
getBestBScStudent    students  =  hd [id \\ {id, level, grades} <- students | level == BSc && (getAvg grades) == maxGrade]
where
    maxGrade :: Real
    maxGrade =  maxList [getAvg grades \\ {level, grades} <- students | level == BSc]
...
/* Arrays. (10 points)
*
* Implement the 'count' function which takes a string and
 counts the repeated characters
...
//countOccurence    ch   str    =  length (filter ((==) ch) [c \\ c <-: str])
countOccurence    ch   str    = length [c \\ c <-: str | c == ch]
//countOccurence    ch   str    =  size {c \\ c <-: str | c == ch}

count :: String -> {(Char, Int)}
count    str    =  {(c, countOccurence c str) \\ c <- uniqueChars | (countOccurence c str) > 1 }
where
	uniqueChars :: [Char]
	uniqueChars =  removeDup [c \\ c <-: str]
...
instance - [Int]
where
	(-) left right = [l \\ l <- left | not (isMember l right)]
...
Insert    (Pointer a ll)      str    =  (Pointer a (Insert ll str))
