...
sortByGrades :: (String, [Int]) (String, [Int]) -> Bool
sortByGrades    (_, left)       (_, right)      =>  (avg (map toReal left)) > (avg (map toReal right))

getBestBScStudent :: [Student] -> String
getBestBScStudent    []        => "ERROR"
getBestBScStudent    students  => fst (hd (sortBy sortByGrades candidates))
where
  candidates :: [(String, [Int])]
  candidates =  [(id, grades) \\ {id, level, grades} <- students | level == BSc]
...
countCh :: Char [Char] -> Int
countCh    ch   str    => length (filter ((==)ch) str)

count :: String -> {(Char, Int)}
count    str    => {(ch, countCh ch lStr) \\ ch <- (removeDup lStr) | (countCh ch lStr) >= 2} 
where 
	lStr :: [Char]
	lStr =	[ch \\ ch <-: str]
...
instance - [Int]
where
  (-) ls rs = [l \\ l <- ls | not (isMember l rs)]
...
llToList    Nil                 => []
llToList    (Pointer x xs)  => [x : (llToList xs)]
...
| x == str                            => xs
| otherwise                           => (Pointer x (delete xs str))
...
concat    Nil                 rs                 => rs
concat    ls                  Nil                => ls
concat    (Pointer l ls)      rs                 => (Pointer l (concat ls rs))
...
where
  (==) left right = left.name == right.name && left.ip == right.ip

hasFake :: BasicPersonAccount [BasicPersonAccount] -> Bool
hasFake    acc                accs                 => (length (filter ((==) acc) accs)) > 1
 
findFakes :: {BasicPersonAccount} -> {BasicPersonAccount}
findFakes    accs                 => { {acc & inQueue=(hasFake acc lAccs)} \\ acc <- (removeDup lAccs)}
where 
  lAccs :: [BasicPersonAccount]
  lAccs =  [acc \\ acc <-: accs]
...
diagSum :: [[Int]] -> Int
diagSum    m       => sum [((m !! i) !! i) \\ i <- (indexList m)]

sortByDiagSum :: [[Int]] [[Int]] -> Bool
sortByDiagSum    left    right   => (diagSum left) > (diagSum right)

largestMat :: {[[Int]]} -> [[Int]]
largestMat    ms        => hd (sortBy sortByDiagSum lMs)
where
  lMs :: [[[Int]]]
  lMs =  [m \\ m <-: ms]
...
llToListB :: (MyList a)   -> [a]
llToListB    Empty        => []
llToListB    (Elem x xs)  => [ x  : (llToListB xs) ]


instance toString (MyList Int)
where
  toString    Empty	= "[]"
  toString    ll    = "[" +++ (toString x) +++ (foldr (+++) "" [ ","+++ toString ch \\ ch <- xs]) +++ "]"
  where
    [x:xs] =  llToListB ll
...
firstAndMiddle    Leaf                      => []
firstAndMiddle    (Node (FirstName a) l r)  => [a] ++ (firstAndMiddle l) ++ (firstAndMiddle r)
firstAndMiddle    (Node (MiddleName a) l r) => [a] ++ (firstAndMiddle l) ++ (firstAndMiddle r)
...
findWhich    [(B _) : xs]   => 1 + (findWhich xs)
