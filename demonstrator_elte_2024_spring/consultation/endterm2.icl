...
shiftCipher   str    n   => {toChar ((((toInt ch) + n) - (toInt 'A')) rem 26) + 'A' \\ ch <-: str}
...
check    x      ys      => and [(getDist x.center y) < x.radius \\ y <-: ys]

getDist ::Point Point 	-> Real
getDist   a		b		=> sqrt (((a.x - b.x)^2.0) + ((a.y - b.y)^2.0)) 
...
instance + Library
where
	(+) libA libB = {lib_name = newName , books = newBooks}
	where
		newName :: String
		newName = 	libA.lib_name +++ libB.lib_name
		newBooks :: {Book}
		newBooks =  LtoA (sort (removeDup ((AtoL libA.books) ++ (AtoL libB.books))))
/*
:: Person = {........}
ps :: [Person]
ps = [P1, P2, P3, P4, P1]
...
instance == Library
where
	(==) libA libB = (sort (removeDup (AtoL libA.books))) == (sort (removeDup (AtoL libB.books)))
	
	/*
	lib1 = [B1, B2, B4, B4] --> [B1, B2, B4] --> [B2, B4, B1]
	lib2 = [B4, B1, B2]     --> [B4, B1, B2] --> [B2, B4, B1]
*/
...
instance + {String}
where
	(+) ls rs = {mergeStrings l r \\ l <-: ls & r <-: rs} 

mergeStrings :: String	String 	-> String
mergeStrings    ""		""		=> ""
mergeStrings    ""		r		=> r
mergeStrings    l		""		=> l
mergeStrings    strL	strR		=> (toString [l, r]) +++ (mergeStrings (toString ls) (toString rs))
where
	[l:ls] 	= AtoL strL
	[r:rs]	= AtoL strR
*/
...
TreeToL :: (Tree a) 		-> [a]
TreeToL		Leaf			=> []
TreeToL		(Node x l r) 	=> (TreeToL l) ++ [x] ++ (TreeToL r)

replaceNodes :: (Tree Int)		[Int] 	-> (Tree Int)
replaceNodes    Leaf			_		=> Leaf
replaceNodes	(Node x l r) 	ys		=> (Node (sum (filter ((>=)x) ys)) (replaceNodes l ys) (replaceNodes r ys)) 

transform :: (Tree Int) -> (Tree Int)
transform    tree 		=> replaceNodes tree (TreeToL tree)
